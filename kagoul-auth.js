...
// Compatible avec une page qui expose #loginFooterBtn et l'événement kagoul:open-auth-modal
...
const DEFAULT_REDIRECT = (() => {
  ...
  if (/login|signin/i.test(p)) return 'index.html';
  return p;
})();
...
function getReturnTo() {
  ...
  const normalizeTarget = (raw) => {
    ...
    try {
      ...
      if (lowerPath.includes('login') || lowerPath.includes('signin')) return 'index.html';
      if (t.origin === window.location.origin) return pathname + t.search + t.hash;
    } catch (e) {
      if (/login|signin/i.test(raw)) return 'index.html';
      return raw;
    }
    return null;
  };
 ...
 * Même logique que la contrainte PostgreSQL : dob <= CURRENT_DATE - INTERVAL '18 years'
 */
...
function createKagoulProfileModal() {
  ...
  overlay.style = `
    ...
    background:rgba(0,0,0,0.60); z-index:99999; padding:20px;
  `;

  const box = document.createElement('div');
  box.style = `
    width:100%; max-width:420px; background:#ffffff;
    border:1px solid #e2e8f0; color:#0f172a; border-radius:12px; padding:24px;
  ...
  box.innerHTML = `
    ...
    <input id="kPseudoInput" type="text" maxlength="32" placeholder="ex: ton_pseudo"
      style="width:100%; padding:10px; border-radius:8px; border:1px solid #e2e8f0;
             background:#f8fafc; color:#0f172a; box-sizing:border-box; font-size:14px;" />
    ...
    <div style="display:flex; gap:8px; margin-top:16px; justify-content:flex-end;">
      <button id="kProfileCancel"
        style="padding:8px 14px; border-radius:8px; background:#f1f5f9;
               border:1px solid #e2e8f0; color:#475569; font-size:14px; cursor:pointer;">
        Annuler
      </button>
  ...
  // Limiter la date max à aujourd'hui (pas de date future)
...
async function ensureProfile(supabase, user) {
  ...
  try {
    ...
    if (selErr) {
      ...
      // Profil complet — vérifier l'âge (double sécurité côté client)
  ...
  modal.btnSave.addEventListener('click', async () => {
    ...
    const dob  = modal.dobInput.value; // format YYYY-MM-DD
    ...
    try {
      ...
      // Contrainte âge rejetée par la DB (failsafe)
      ...
      // Conflit user_id → fallback update
...
async function handleUnderageAccess(supabase, user) {
  ...
  // Supprimer le profil si déjà inséré (cas anormal — failsafe)
  try {
  ...
  // Afficher un écran de blocage
  document.body.innerHTML = `
    <div style="
      min-height:100vh; display:flex; align-items:center; justify-content:center;
      background:#0f172a; font-family:system-ui,sans-serif; padding:20px;
    ">
      <div style="max-width:400px; text-align:center; color:#f8fafc;">
        <div style="font-size:48px; margin-bottom:16px;">🔞</div>
        <h1 style="font-size:24px; font-weight:700; margin-bottom:12px;">Accès refusé</h1>
        <p style="color:#94a3b8; font-size:15px; line-height:1.6;">
          Kagoul est une plateforme réservée aux personnes ayant au moins <strong>18 ans</strong>.
    ...
    <div id="authContent" style="margin-top:14px;">
      ...
      <div id="loginPane">
        ...
        <input id="loginEmail" type="email" placeholder="you@example.com"
          style="width:100%; padding:10px; border-radius:8px; border:1px solid #e6e9ee; margin-top:6px; box-sizing:border-box;" />
        ...
        <div style="margin-top:12px; display:flex; gap:8px;">
          <button id="loginBtn"
            style="flex:1; padding:10px; border-radius:8px; background:#7c3aed; color:#fff; border:none; font-weight:600; cursor:pointer;">
            Se connecter
          </button>
          <button id="googleLoginBtn"
            style="flex:0 0 48px; border-radius:8px; border:1px solid #e6e9ee; background:#fff; cursor:pointer;" title="Se connecter avec Google">
      ...
      <div id="signupPane" style="display:none;">
        ...
        <input id="signupEmail" type="email" placeholder="you@example.com"
          style="width:100%; padding:10px; border-radius:8px; border:1px solid #e6e9ee; margin-top:6px; box-sizing:border-box;" />
        <label style="display:block; font-size:13px; color:#334155; margin-top:10px;">Mot de passe</label>
        <input id="signupPass" type="password" placeholder="Mot de passe (min 6)"
          style="width:100%; padding:10px; border-radius:8px; border:1px solid #e6e9ee; margin-top:6px; box-sizing:border-box;" />
        ...
        <div style="margin-top:12px; display:flex; gap:8px;">
          ...
          <button id="googleSignupBtn"
            style="flex:0 0 48px; border-radius:8px; border:1px solid #e6e9ee; background:#fff; cursor:pointer;" title="S'inscrire avec Google">
            <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" alt="Google"
              style="width:20px;height:20px;display:block;margin:auto;" />
          </button>
        </div>
...
async function init() {
  ...
  function bindOpeners() {
    ...
    // Événement spécifique à Kagoul (ne pas écraser l'event taktik)
    document.removeEventListener('kagoul:open-auth-modal', window.openAuthModal);
    document.addEventListener('kagoul:open-auth-modal', () => {
  ...
  try {
    ...
    // Marquer offline à la fermeture — k_profiles
...
/* -------------------------
   LEGACY: handlers pour pages avec formulaires inline (login.html)
   ------------------------- */
