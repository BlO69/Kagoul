...
function getReturnTo() {
  ...
  const normalizeTarget = (raw) => {
    ...
    try {
      ...
      // Accepter uniquement les URLs du domaine Kagoul
...
async function handleUnderageAccess(supabase, user) {
  ...
  // Écran de blocage — même charte que la modale
...
function wireAuthModal(supabase, modal) {
  ...
  // GOOGLE OAuth — redirige toujours vers kagoul.netlify.app
  ...
  try {
    ...
    if (googleBtn) {
      googleBtn.addEventListener('click', async (ev) => {
        ev.preventDefault();
        // Toujours rediriger vers kagoul.netlify.app après Google OAuth
